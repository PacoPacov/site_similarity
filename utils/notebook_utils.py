...
def create_node2vec_model(lvl_data, is_weighted, dimension=None, file_name=None, prefix=None, dimensions=[]):
    ...
    if not file_name:
        ...
        file_names = [f"{prefix}_{weight}_{str(dimension)}_D.model" for dimension in dimensions]
    else:
...
def load_note2vec_model(model_name):
...
def eval_node2vec_models(models, data_year):
    ...
    for model in models:
        ...
        node2vec_model = load_note2vec_model(model)
